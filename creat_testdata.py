...
def create_labels(path):
    ...
    labels_one_hot = list(map(lambda x: create_label(label=x, label_set=accusation_type_list), accusation_all))
    np.save('./data_deal/test_big_labels_accusation.npy', labels_one_hot)
...
def fact_cut(path):  # 文字转换
    for i in range(4):  # 分批处理大文件
        ...
        with open('./data_deal/test/data_cut/big_fact_cut_%d_%d.pkl' % (i * 10000, i * 10000 + 10000), mode='wb') as f:
            pickle.dump(big_fact_cut, f)
        print('finish big_fact_cut_test_%d_%d' % (i * 10000, i * 10000 + 10000))
...
def tokenizer_to_sequence(num_words=80000, maxlen=400):
    ...
    for i in range(4):
        ...
        with open('./data_deal/test/data_cut/big_fact_cut_%d_%d.pkl' % (i * 10000, i * 10000 + 10000),
                  mode='rb') as f:
        ...
        with open('./data_deal/test/fact_seq/fact_seq_%d_%d.pkl' % (i * 10000, i * 10000 + 10000), mode='wb') as f:
    ...
    for i in range(4):
        print('start big_fact_cut_%d_%d' % (i * 10000, i * 10000 + 10000))
        with open('./data_deal/test/fact_seq/fact_seq_%d_%d.pkl' % (i * 10000, i * 10000 + 10000), mode='rb') as f:
        ...
        with open('./data_deal/test/fact_pad_seq/fact_pad_seq_%d_%d_%d.pkl' % (maxlen, i * 10000, i * 10000 + 10000),
                  mode='wb') as f:
    ...
    np.save('./data_deal/test_big_fact_pad_seq_%d_%d.npy' % (num_words, maxlen), fact_pad_seq)

def split_train_test(num_words = 80000, maxlen = 800):
    # fact数据集
    ...
    del fact
    np.save('./data_deal/fact_train.npy', fact_train)
    np.save('./data_deal/fact_test.npy', fact_test)

    # 标签数据集

    labels_train, labels_test = train_test_split(labels, test_size=0.05, random_state=1)
    del labels
    np.save('./data_deal/labels_train.npy', labels_train)
    np.save('./data_deal/labels_test.npy', labels_test)
